...
実際にアプリケーションを運用する中で、Azure for Students の特典により無料だと想定していた Azure DataBase for MySQL が、一定の**I/O（入出力）**操作を超えると従量課金されることに気づきました。これが将来的なコスト面の課題になると考え、まずパフォーマンス改善を通して、この課題に取り組みたいと考えています。
